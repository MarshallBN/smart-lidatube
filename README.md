...
Set `SMART_AUDIT_ENABLED=true` to enable it. Audit throughput adapts to backlog and is capped by `SMART_AUDIT_MAX_PER_HOUR=300` (effective rate is the lower of tier and max); existing `SMART_AUDIT_VERIFY_BUDGET_PER_HOUR` values are honored as a fallback ceiling when the new setting is absent. Production gates use Lidarr's read-only status endpoint plus conservative `/proc` load and disk-I/O thresholds (`SMART_AUDIT_MAX_LOAD_PER_CPU=0.75`, `SMART_AUDIT_MAX_DISK_IO_MS=250`) and fail closed. Token carryover remains capped by `SMART_AUDIT_MAX_TOKEN_BANK=24`. The auditor yields whenever a normal retry/import/review job exists and reserves every fifth eligible selection for the longest-unchecked track (`SMART_AUDIT_FAIRNESS_SHARE=0.20`). Candidate discovery stays disabled (`SMART_AUDIT_CANDIDATE_SEARCH_BUDGET_PER_HOUR=0`). Inspect aggregate progress through authenticated `GET /api/smart/audit/status`.

Automatic retry jobs use bounded exponential backoff (at most hourly) until their durable 24-hour deadline. `SMART_MAX_ATTEMPTS` continues to cap manual retries; automatic retries are deadline-bound by default and may be independently capped with `SMART_AUTO_MAX_ATTEMPTS`.

Milestone A adds `/smart-control` and authenticated aggregate/control APIs at `/api/smart/dashboard/summary`, `/quality`, `/jobs`, `/reviews`, and `/events`. Review pagination uses an opaque `next_cursor` keyed by the time an attempt became actionable plus its ID; clients must pass it back unchanged, which allows older attempts that later become actionable to appear on subsequent pages. Review limits remain bounded to 1–200 safe DTOs. Set `SMART_VERSION` for source identification and optionally `SMART_FFPROBE_TIMEOUT`. `SMART_AUDIT_MODE` is server-restricted to `observe`, `review`, or `paused`; paused mode prevents new checks while the normal worker reconciliation loop continues. Candidate discovery remains `0` by default and automatic auditor upgrades are explicitly disabled; `auto_safe` is not accepted. The control page keeps its bearer token only in JavaScript memory and never embeds or persistently stores it.
