...
Set `NAVIDROME_MUSIC_ROOT` and `LIDARR_MUSIC_ROOT` when their library mount roots differ. Track-file lookup is paginated and ambiguous suffix matches fail closed. Claimed work has a `SMART_CLAIM_TIMEOUT` lease. Submitted imports remain `importing` across restarts and are reconciled rather than submitted twice; an async HTTP 202 is never treated as completion. Rejected candidate directories are retained for diagnosis; verified imports are cleaned. Manual review requires valid Telegram configuration; otherwise jobs report an actionable unavailable/pending status.
